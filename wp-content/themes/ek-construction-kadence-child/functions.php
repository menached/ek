 ...
 */
function orbisius_ct_ek_construction_kadence_child_child_theme_enqueue_styles()
{
    global $wp_styles;
...
add_action('wp_enqueue_scripts', 'orbisius_ct_ek_construction_kadence_child_child_theme_enqueue_styles', 25);

function orbisius_ct_ek_construction_kadence_child_example_function()
{
    // put some code here
}
...
// Enqueue parent and child theme styles as usual

add_action('wp_enqueue_scripts', function () {
    // Parent theme style
    wp_enqueue_style('parent-style', get_template_directory_uri() . '/style.css');
    // Child theme style
    wp_enqueue_style('ekc-child-style', get_stylesheet_uri(), [ 'parent-style' ], wp_get_theme()->get('Version'));

    // Enqueue modular CSS (base-fonts, etc.) from child theme directory
    wp_enqueue_style(
        'ekc-base-fonts',
        get_stylesheet_directory_uri() . '/base-fonts.css',
        [ 'ekc-child-style' ],
        filemtime(get_stylesheet_directory() . '/base-fonts.css')
    );
    // Example: add more modular CSS files in this pattern
    // wp_enqueue_style('ekc-faq-style', get_stylesheet_directory_uri() . '/faq.css', [ 'ekc-child-style' ], filemtime(get_stylesheet_directory() . '/faq.css'));
    wp_enqueue_style('ekc-lists', get_stylesheet_directory_uri() . '/lists.css', [], filemtime(get_stylesheet_directory() . '/lists.css'));
});
...
add_action('wp_enqueue_scripts', function () {
    // Enqueue parent and child styles as before...
    wp_enqueue_style('parent-style', get_template_directory_uri() . '/style.css');
    wp_enqueue_style('child-style', get_stylesheet_uri(), [ 'parent-style' ], wp_get_theme()->get('Version'));
    // Enqueue modular forms styles
    wp_enqueue_style('ekc-forms', get_stylesheet_directory_uri() . '/css/forms.css', [ 'child-style' ], filemtime(get_stylesheet_directory() . '/css/forms.css'));
});
